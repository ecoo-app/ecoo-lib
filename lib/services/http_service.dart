...
class HTTPService {
  ...
  HTTPService(this.baseURL, {this.session, http.Client client})
  ...
  Future<dynamic> _doGet(String path,
      {int iteration = 0, int maxRetries = 1}) async {
    try {
      ...
      if (session != null && e.statusCode == 401) {
        ...
        return await _doGet(path,
            iteration: iteration + 1, maxRetries: maxRetries);
      }
      if (e.statusCode == 401) {
        throw NotAuthenticatedError();
      } else {
  ...
  Future<dynamic> _doPost(String path, dynamic body,
      {int iteration = 0, int maxRetries = 1}) async {
    try {
      final url = _url(path: path);
      final headers = _requestHeaders();
      ...
      final response =
          await _httpClient.post(url, headers: headers, body: jsonEncode(body));
      if (kDebugMode) {
        print("Response status: ${response.statusCode}");
        print("Response body: ${response.body.toString()}");
      }
  ...
  Future<dynamic> _doDelete(String path,
      {int iteration = 0, int maxRetries = 1}) async {
    try {
      final url = _url(path: path);
      final headers = _requestHeaders();
  ...
  Map<String, String> _requestHeaders() {
    ...
    if (session != null && session.token != null) {
      result[HttpHeaders.authorizationHeader] =
