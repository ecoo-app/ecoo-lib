...
class Storage(private val context: Context, private val storageAlias: String, private var isParanoia: Boolean = false) {
    ...
    fun readString(fileKey: String, success: (String) -> Unit, error: (Exception) -> Unit, requestAuthentication: (() -> Unit) -> Unit) {
        when {
            keyStore.containsAlias(keyStoreAlias) -> {
                ...
                error(ItemCorruptedException())
            }
        }
    }

    fun readString(fileKey: String, onAuthenticationRequired: (() -> Unit) -> Unit, completion: (result: Result<String>) -> Unit) {
        readString(fileKey, { completion(Result.success(it)) }, { completion(Result.failure(it)) }, onAuthenticationRequired)
    }
    ...
    fun writeString(fileKey: String, fileData: String, onAuthenticationRequired: (() -> Unit) -> Unit, completion: (result: Result<Unit>) -> Unit) {
        writeString(fileKey, fileData, { completion(Result.success(Unit)) }, { Result.failure<Exception>(it) }, onAuthenticationRequired)
    }
    ...
    private fun retrievePasswordSecretKeySpec(passwordFile: SecureFile, passphraseOrPin: String): SecretKeySpec {
        ...
        for (i in secretKeyDigest.indices) {
            if (secretKeyDigest[i] != decryptedContent[32 + i]) {
                throw DigestNotMatchingException()
            }
        }
