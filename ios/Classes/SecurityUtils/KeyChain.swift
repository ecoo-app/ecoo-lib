    ...
    public class PrivateKey {
        ...
        public init(tag: Data, accessControl: SecAccessControlCreateFlags = [], protection: Keychain.Protection = .whenPasscodeSetThisDeviceOnly) throws {
            ...
            guard let key = SecKeyCreateRandomKey(attributes as CFDictionary, &error) else {
                if let error = error?.autorelease().takeUnretainedValue(), CFErrorGetDomain(error) == kCFErrorDomainOSStatus {
                    let code = CFErrorGetCode(error)
                    throw Keychain.Error.osStatus(OSStatus(code))
                } else {
                    throw Keychain.Error(error?.autorelease().takeUnretainedValue())
                }
            }
            ...
            guard status.isSuccess || status == errSecDuplicateItem else {
    ...
    public enum Error: Swift.Error {
        ...
        var isAuthError: Bool {
            if case let .osStatus(status) = self {
                return status == errSecAuthFailed || status == errSecNotAvailable
            } else {
                return false
            }
        }
